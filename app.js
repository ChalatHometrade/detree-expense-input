(function () {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (SAVE_MODE === "google_sheets") {
        const localRecord = createLocalRecord(row, "sending");
        ...
        if (isOffline()) {
          markRecordFailed(localRecord._id, new Error("อุปกรณ์ออฟไลน์"));
          showErrorMessage("อุปกรณ์ออฟไลน์ ข้อมูลถูกเก็บในเครื่องแล้ว กรุณากดส่งซ้ำเมื่อมีอินเทอร์เน็ต");
          return;
        }

        sendLocalRecordToGoogleSheet(localRecord._id);
  ...
  async function saveExpense(row) {
    ...
    if (SAVE_MODE === "google_sheets") {
      await submitExpenseToGoogleSheet(getExpensePayload(row));
      saveLocalRecord(createLocalRecord(row, "sent"));
      return;
    }
  ...
  function createLocalRecord(row, status = "sending") {
    return {
      ...getExpensePayload(row),
      _id: createRecordId(),
      _syncStatus: status,
  ...
  function loadExpenses() {
    try {
      ...
      let needsSave = false;
      const expenses = parsed.map((record) => {
        if (record._id) return record;
        needsSave = true;
        return ensureLocalRecordId(record);
      });

      if (needsSave) saveExpenses(expenses);
      return expenses;
    } catch (error) {
  ...
  function getLocalRecordById(recordId) {
    return loadExpenses().find((record) => record._id === recordId) || null;
  }

  function updateLocalRecord(recordId, patch) {
    const expenses = loadExpenses();
    let didUpdate = false;
    const updatedExpenses = expenses.map((record) => {
      const recordWithId = ensureLocalRecordId(record);
      if (recordWithId._id !== recordId) return recordWithId;
  ...
  function ensureLocalRecordId(record) {
    if (record._id) return record;
  ...
  function markRecordSending(recordId) {
  ...
  function markRecordSent(recordId) {
    updateLocalRecord(recordId, {
      _syncStatus: "sent",
      _lastSyncAttemptAt: new Date().toISOString(),
      _syncError: ""
  ...
  function markRecordFailed(recordId, error) {
    updateLocalRecord(recordId, {
      _syncStatus: "failed",
      _lastSyncAttemptAt: new Date().toISOString(),
  ...
  function getExpensePayload(record) {
    return {
      "ปี": record["ปี"],
      "เดือน": record["เดือน"],
      "วันที่": record["วันที่"],
      "รายการสินค้า": record["รายการสินค้า"],
      "ราคา": record["ราคา"],
      "หมวดหมู่": record["หมวดหมู่"]
    };
  }

  function getSyncStatus(record) {
    return record._syncStatus || "sent";
  }
  ...
  function getSyncStatusLabel(status) {
    ...
    return labels[status] || "ส่งแล้ว";
  }

  function sendLocalRecordToGoogleSheet(recordId) {
    const record = getLocalRecordById(recordId);
    if (!record) return;

    const payload = getExpensePayload(record);
    submitExpenseToGoogleSheet(payload)
      .then(() => {
        markRecordSent(recordId);
      })
      .catch((error) => {
        markRecordFailed(recordId, error);
        showErrorMessage("ส่งเข้า Google Sheet ไม่สำเร็จ แต่ข้อมูลยังอยู่ในเครื่อง กรุณากดส่งซ้ำ");
      });
  }

  function retrySync(recordId) {
    markRecordSending(recordId);
    renderExpenseTable();
    clearErrors();
    showStatus("กำลังส่งซ้ำเข้า Google Sheet...");

    if (isOffline()) {
      markRecordFailed(recordId, new Error("อุปกรณ์ออฟไลน์"));
      showErrorMessage("อุปกรณ์ออฟไลน์ ข้อมูลถูกเก็บในเครื่องแล้ว กรุณากดส่งซ้ำเมื่อมีอินเทอร์เน็ต");
      return;
    }

    sendLocalRecordToGoogleSheet(recordId);
  }
  ...
  function renderExpenseTable() {
    ...
    visibleExpenses.forEach(({ row, originalIndex }) => {
      ...
      statusBadge.className = `sync-badge sync-badge-${status}`;
      statusBadge.textContent = getSyncStatusLabel(status);
      statusCell.appendChild(statusBadge);

      if (status === "failed" && row._id) {
        const retryButton = document.createElement("button");
        retryButton.type = "button";
        retryButton.className = "retry-row-button";
        retryButton.textContent = "ส่งซ้ำ";
        retryButton.addEventListener("click", () => retrySync(row._id));
